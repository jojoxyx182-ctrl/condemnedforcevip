...
header() {
    clear
    echo -e "\n  ${L_PILL}${BOLD} CONDEMNED FORCE ${RESET}${R_PILL}${BOLD} HOSTING MANAGER ${RESET} ${DIM}${RESET}"
    echo -e "  ${GRAY}──────────────────────────────────────────────────────${RESET}"
}
...
while true; do
    ...
    case "$c" in
        1) bash <(curl -fsSL https://raw.githubusercontent.com/jojoxyx182-ctrl/condemnedforcevip/refs/heads/main/panel.sh) ;;
        2) bash <(curl -fsSL https://raw.githubusercontent.com/jojoxyx182-ctrl/condemnedforcevip/refs/heads/main/node.sh) ;;
        3) bash <(curl -fsSL https://raw.githubusercontent.com/jojoxyx182-ctrl/condemnedforcevip/refs/heads/main/tema.sh) ;;
        4) bash <(curl -fsSL https://raw.githubusercontent.com/jojoxyx182-ctrl/condemnedforcevip/refs/heads/main/443) ;;
        5) show_all_info ;;
