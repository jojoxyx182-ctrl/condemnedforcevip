...
# =====================================================
# CONDEMNEDFORCE XYZ - ENHANCED MULTI-VM MANAGER
# =====================================================
...
display_header() {
    clear
    echo -e "\n  ${L_PILL}${BOLD} FORCEXYZ ${RESET}${R_PILL}${BOLD} VM MANAGER ${RESET}"
    echo -e "  ${GRAY}──────────────────────────────────────────────────────${RESET}"
    echo
}
...
print_status() {
    ...
    case $type in
        ...
        "WARN") echo -e "${ERROR}⚠️  [WARN]${RESET} $message" ;;
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        if [[ "$GUI_MODE" == true ]]; then
            ...
            print_status "INFO" "⚠️ Press Ctrl+A then X to exit QEMU console"
        fi
        ...
        print_status "INFO" "⚠️ VM $vm_name has been shut down"
    fi
}
...
stop_vm() {
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            print_status "INFO" "⚠️ Stopping VM: $vm_name"
    ...
    while true; do
        ...
        if [ $vm_count -gt 0 ]; then
            ...
            for i in "${!vms[@]}"; do
                ...
                printf "  ${MAIN}${BOLD}%2d)${RESET} ${WHITE}%s %s${RESET}\n" $((i+1)) "${vms[$i]}" "$status"
            done
            echo
        fi
        
        echo "  ${BOLD}${WHITE}MAIN MENU${RESET}"
        echo
        ...
        case $choice in
            ...
            3)
                if [ $vm_count -gt 0 ]; then
                    read -p "$(print_status "INPUT" "⚠️ Enter VM number to stop: ")" vm_num
                    if [[ "$vm_num" =~ ^[0-9]+$ ]] && [ "$vm_num" -ge 1 ] && [ "$vm_num" -le $vm_count ]; then
            ...
            9)
                ...
                ;;
            0)
                print_status "INFO" "👋 Thanks for using this tool! 🌟"
                exit 0
                ;;
            *)
